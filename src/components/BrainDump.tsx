...
export function BrainDump({ onTasksCreated }: BrainDumpProps) {
    ...
    const handleSubmit = async () => {
        const lines = input
            .split('\n')
            .map(l => l.trim().slice(0, 500))  // Truncate each line to 500 chars
            .filter(l => l.length > 0);
