...
async function importStaffSheet(
  ...
  for (const row of rows) {
    ...
    for (const [colName, value] of Object.entries(row)) {
      ...
      if (field === 'is_paid') {
        (payPeriod as unknown as Record<string, unknown>)[field] =
          value === true || value === 'yes' || value === 'Yes' || value === 'YES' || value === 1;
      } else if (field === 'notes') {
        (payPeriod as unknown as Record<string, unknown>)[field] = String(value);
      } else {
        const num = typeof value === 'number' ? value : parseFloat(String(value));
        if (!isNaN(num)) {
          (payPeriod as unknown as Record<string, unknown>)[field] = num;
        }
      }
    }
  ...
  for (const row of rows) {
    ...
    for (const [colName, value] of Object.entries(row)) {
      ...
      if (field === 'vendor' || field === 'channel' || field === 'notes') {
        (expense as unknown as Record<string, unknown>)[field] = String(value);
      } else if (field === 'date') {
        // Already handled
      } else {
        const num = typeof value === 'number' ? value : parseFloat(String(value));
        if (!isNaN(num)) {
          (expense as unknown as Record<string, unknown>)[field] = num;
        }
      }
    }
